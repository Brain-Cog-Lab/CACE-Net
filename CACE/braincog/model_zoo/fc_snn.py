...
class SHD_SNN(BaseModule):
    ...
    def __init__(self,
        ...
        self.ts_conv = STSC(input_dim//1, dimension=2, time_rf_conv=5, time_rf_at=3, use_gate=True, use_filter=True)
        self.fc = nn.Sequential(
            nn.Linear(input_dim//1, hidden_size),
            nn.Dropout(0.1),
    ...
    def forward(self, inputs):
        inputs = inputs.permute(1, 0, 2).contiguous() # (b, t, c) -> (t, b, c)
        self.reset()

        outputs = []
        if self.layer_by_layer:
            for t in range(inputs.shape[0]):
                x = inputs[t].unsqueeze(0).repeat(self.step, 1, 1)
                # x = self.ts_conv(x)
                x = rearrange(x, 't b c -> (t b) c', t=self.step)
                x = self.fc(x)
                x = rearrange(x, '(t b) c -> t b c', t=self.step)
                outputs.append(torch.mean(x, dim=0))
...
if __name__ == "__main__":
    net = SHD_SNN(step=10, tau=2.0, threshold=0.5, layer_by_layer=True, input_dim=128).cuda()
    x = torch.randn((64, 10, 128)).cuda()
    res = net(x)
    print(res.shape)