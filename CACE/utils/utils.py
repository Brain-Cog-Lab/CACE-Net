...
def Prepare_logger(args, eval=False):
    ...
    logfile = args.snapshot_pref+date+'_seed_'+str(args.seed)+'_contrastive_'+str(args.contrastive)+'_guide_'+str(args.guide)+'_lambda_'+str(args.Lambda) + '_contras-coeff_'+str(args.contras_coeff) +'_psai_'+str(args.psai)+'.log' if not eval else args.snapshot_pref + f'/{date}-Eval.log'
