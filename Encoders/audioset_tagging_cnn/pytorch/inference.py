    ...
    with open('/home/hexiang/Encoders/data/AVE_Dataset/Annotations.txt', 'r') as file:
        for line in file:
            ...
            if len(parts) > 1:  # 确保有足够的部分来提取文件名
                file_name = parts[1]
                lis.append(file_name + '.wav')

    audio_features = np.zeros([len(lis), 10, 2048])  # 10s long audio

    for t in tqdm(range(len(lis)), total=len(lis)):
        audio_path = "/home/hexiang/Encoders/data/AVE_audio/AVE_wav/" + lis[t]
        # Load audio
        ...
        if 'embedding' in batch_output_dict.keys():
            ...
            audio_features[t, :, :] = embedding.astype(np.float64)

    with h5py.File('/home/hexiang/Encoders/data/audio_pytorch_librosa_cnn14_feature.h5', 'w') as hf:
